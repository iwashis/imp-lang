...
genCommExpr = oneof 
    ...
    {-
-- Define a generator for all expressions
genExpr :: Gen (Expr a)
genExpr = oneof 
    [ coerce (Var <$> arbitrary)
    , coerce (Constant <$> arbitrary)
    , coerce (Op2 <$> arbitrary <*> genArithmeticExpr <*> genArithmeticExpr)
    , coerce $ pure T
    , coerce $ pure F
    , coerce $ LessOrEq <$> genArithmeticExpr <*> genArithmeticExpr
    , coerce $ pure Skip
    , coerce $ Assign <$> arbitrary <*> genArithmeticExpr
    , coerce $ AndThen <$> genExpr <*> genExpr
    , coerce $ IfElse <$> genBoolExpr <*> genExpr <*> genExpr
    , coerce $ While <$> genBoolExpr <*> genExpr
    ]
